...
| `PanelPreprocess6_WorkerDayPanel.py` | Creates the anonymized selected-column worker-day panel from restricted/intermediate inputs. |
| `Utils/` | Helper code for climate processing, order JSON processing, and human heat-balance calculations. |
...
If Stata reports missing dependencies for `reghdfe`, install the packages requested by Stata, commonly `ftools`.
...
The figure notebooks use the processed panel, Stata outputs, supplementary CSV files, and counterfactual files already included in the archive. Execute the notebooks in figure order:
...
Some notebooks contain high-resolution figure outputs and may take longer to run.

### Optional Rider-Tier Calibration Check

The auxiliary script `Utils/tier_calibration_forSI2.1.3.py` documents the SI 2.1.3 rider-tier calibration check. It requires `SI/rider_state.xlsx`, which contains rider-level z-scores for attendance days, attendance rate, and average daily orders, together with the observed rider level. When run, the script performs a normalized weight grid search, identifies the best agreement with the observed Elite label, evaluates nearby high-agreement weight sets, applies weight-perturbation sensitivity checks, and reports group-separation and ROC diagnostics.

```powershell
python Utils/tier_calibration_forSI2.1.3.py
```

The script prints diagnostics to the console and does not create additional output files.
...
Before sharing or archiving the repository, a lightweight Python syntax check can be run:

```powershell
python -m py_compile PanelPreprocess1_Order.py PanelPreprocess2_Weather.py PanelPreprocess3_ThermalRisk.py PanelPreprocess4_Counterfactual.py PanelPreprocess5_DD.py PanelPreprocess6_WorkerDayPanel.py Utils/climateProcess.py Utils/orderJsonProcess.py Utils/rider_character.py
```

Notebook outputs may contain local execution paths generated by warnings or printed `Path` objects. For a clean public archive, clear notebook outputs and rerun them in a clean environment if needed.
